...
export default class Whiteboard extends Component {
    ...
    handleShareScreen = () => {
        if (!this.state.isSharing) {
            let list = this._rtc.getScreenWindowsInfo();
                ...
                <div className="board" id="whiteboard" style={{ display: this.state.isSharing ? 'none' : 'block' }}>
                    {
                        this.state.whiteReadyState === false ? (
                            ...
                            : (
                                <React.Fragment>
                                    <div style={{ display: this.state.role === 'audience' ? 'flex' : 'none' }} className="board-mask"></div>
                                    <RoomWhiteboard room={this.state.room} style={{ width: '100%', height: '100vh' }} />
